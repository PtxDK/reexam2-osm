...
#include "proc/cond.h"      // sizeof(cond_t)
...
void init_startup_thread(uint32_t arg)
{
  ...
  KERNEL_ASSERT(sizeof(mutex_t) == 8); // proc/types.h assumes 64 bits.
  KERNEL_ASSERT(sizeof(cond_t) == 4); // proc/types.h assumes 32 bits.
  ...
  pid = process_spawn(bootargs_get("initprog"), NULL);
  ...
  if (pid < 0) {
    ...
    /* The current initial process should never terminate. */
    KERNEL_PANIC("Initial process terminated.\n");
  }
}
