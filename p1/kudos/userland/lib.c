 ...
 * 'func'. Returns 0 on success or a negative value on error.
 */
int syscall_thread(void (*func)(void*), void *arg)
{
  return (int)_syscall(SYSCALL_THREAD, (uintptr_t)func, (uintptr_t)arg, 0);
}
...
{
  return (int)_syscall(SYSCALL_GETTID, 0, 0, 0);
}
...
{
  (int)_syscall(SYSCALL_MUTEX_INIT, (uintptr_t)mutex, 0, 0);
}
...
{
  (int)_syscall(SYSCALL_MUTEX_LOCK, (uintptr_t)mutex, 0, 0);
}
...
{
  (int)_syscall(SYSCALL_MUTEX_UNLOCK, (uintptr_t)mutex, 0, 0);
}
...
{
  _syscall(SYSCALL_COND_WAIT, (uintptr_t)cond, (uintptr_t)mutex, 0);
}
...
{
  _syscall(SYSCALL_COND_SIGNAL, (uintptr_t)cond, 0, 0);
}
...
{
  _syscall(SYSCALL_COND_BROADCAST, (uintptr_t)cond, 0, 0);
}
...
int putc(char c)
{
  return fputc((FILE*)FILEHANDLE_STDOUT, c);
}
...
int getc(void)
{
  return fgetc((FILE *)FILEHANDLE_STDIN);
}
